...
from langgraph_eval.models import EvalCase
from langgraph_eval.optimizers import DSpyOptimizerAdapter
...
async def test_dspy_adapter_compiles_and_records_candidate(
    ...
    monkeypatch.setattr(
        "langgraph_eval.optimizers.importlib.import_module", lambda name: fake_dspy
    )
...
async def test_dspy_adapter_bounds_serialized_candidate_state(
    ...
    monkeypatch.setattr(
        "langgraph_eval.optimizers.importlib.import_module", lambda name: ModuleType("dspy")
    )
    adapter = DSpyOptimizerAdapter(
        candidate_id="candidate",
        program_factory=lambda dspy: "student",
