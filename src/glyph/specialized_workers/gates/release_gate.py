...
class ReleaseGate:
    ...
    async def evaluate_release(
        ...
        *,
        ...
        security_pass_rate = security_summary.pass_rate if security_summary is not None else 0.0

        # Calculate overall error rate
        total_trials = deterministic_summary.total
        error_rate = deterministic_summary.errors / total_trials if total_trials > 0 else 0.0

        # Make final decision
        all_required_passed = (
            (not self.policy.require_deterministic or det_passed)
            and (not self.policy.require_regression_check or reg_passed)
            and (not self.policy.require_judge or judge_passed)
        )
    ...
    def create_strict_policy(self) -> ReleasePolicy:
        ...
        from glyph.specialized_workers.policy_registry import release_policy_from_profile

        return release_policy_from_profile("strict")

    def create_development_policy(self) -> ReleasePolicy:
        """Create a lenient release policy for development iterations."""
        from glyph.specialized_workers.policy_registry import release_policy_from_profile

        return release_policy_from_profile("development")

    def create_staging_policy(self) -> ReleasePolicy:
        """Create a balanced release policy for staging environments."""
        from glyph.specialized_workers.policy_registry import release_policy_from_profile

        return release_policy_from_profile("staging")
